...
With Me.Text1
.Locked = True
.Text = "您可以通過本工具在將文件移動到其它位置的同時在源位置創建一個0字節的副本(影子文件)，儅目標位置失效(例如：將文件保存的可移動存儲裝置被拔出)后，您仍然可以索引這個文件。您可以選擇是否將目標位置寫入影子文件中。支持批量操作。請確保您對要操作的文件具有完全控制權限。"
End With
With Me
