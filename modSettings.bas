...
PromptUser = 2
End Enum
Public lpErrorOperation As ErrorOperations
...
Dim CommonDialog1 As New CCommonDialog
 ...
 dwSize As Long
 cntUsage As Long
 th32ProcessID As Long '½ø³ÌID
 th32DefaultHeapID As Long '¶ÑÕ»ID
 th32ModuleID As Long 'Ä£¿éID
 cntThreads As Long
 th32ParentProcessID As Long '¸¸½ø³ÌID
...
Private liOldSystem As LARGE_INTEGER
 ...
 Private Declare Function CreateToolhelp32Snapshot Lib "kernel32" (ByVal dwFlags As Long, ByVal th32ProcessID As Long) As Long '»ñÈ¡Ê×¸ö½ø³Ì
 Private Declare Function Process32First Lib "kernel32" (ByVal hSnapshot As Long, lppe As PROCESSENTRY32) As Long '»ñÈ¡ÏÂ¸ö½ø³Ì
 Private Declare Function Process32Next Lib "kernel32" (ByVal hSnapshot As Long, lppe As PROCESSENTRY32) As Long 'ÊÍ·Å¾ä±ú
...
'ºÜ¶àÅóÓÑ¶¼¼ûµ½¹ýÄÜÔÚÍÐÅÌÍ¼±êÉÏ³öÏÖÆøÇòÌáÊ¾µÄÈí¼þ£¬²»ËµÈí¼þ£¬¾ÍÊÇÔÚ¡°´ÅÅÌ¿Õ¼ä²»×ã¡±Ê±Windows¸ø³öµÄÌáÊ¾¾ÍÊôÓÚÆøÇòÌáÊ¾£¬ÄÇÃ´ÔõÑùÔÚ×Ô¼ºµÄ³ÌÐòÖÐÌí¼ÓÕâÑùµÄÆøÇòÌáÊ¾ÄØ£¿
   
'ÆäÊµ²¢²»ÄÑ£¬¹Ø¼ü¾ÍÔÚÌí¼ÓÍÐÅÌÍ¼±êÊ±ËùÊ¹ÓÃµÄNOTIFYICONDATA½á¹¹£¬Ô´´úÂëÈçÏÂ£º
Private Declare Function Shell_NotifyIcon Lib "shell32.dll" Alias "Shell_NotifyIconA" (ByVal dwMessage As Long, lpData As NOTIFYICONDATA) As Long
   
Private Type NOTIFYICONDATA
cbSize   As Long     '   ½á¹¹´óÐ¡(×Ö½Ú)
hWnd   As Long     '   ´¦ÀíÏûÏ¢µÄ´°¿ÚµÄ¾ä±ú
uID   As Long     '   Î¨Ò»µÄ±êÊ¶·û
uFlags   As Long     '   Flags
uCallbackMessage   As Long     '   ´¦ÀíÏûÏ¢µÄ´°¿Ú½ÓÊÕµÄÏûÏ¢
hIcon   As Long     '   ÍÐÅÌÍ¼±ê¾ä±ú
szTip   As String * 128         '   Tooltip   ÌáÊ¾ÎÄ±¾
dwState   As Long     '   ÍÐÅÌÍ¼±ê×´Ì¬
dwStateMask   As Long     '   ×´Ì¬ÑÚÂë
szInfo   As String * 256         '   ÆøÇòÌáÊ¾ÎÄ±¾
uTimeoutOrVersion   As Long     '   ÆøÇòÌáÊ¾ÏûÊ§Ê±¼ä»ò°æ±¾
'   uTimeout   -   ÆøÇòÌáÊ¾ÏûÊ§Ê±¼ä(µ¥Î»:ms,   10000   --   30000)
'   uVersion   -   °æ±¾(0   for   V4,   3   for   V5)
szInfoTitle   As String * 64         '   ÆøÇòÌáÊ¾±êÌâ
dwInfoFlags   As Long     '   ÆøÇòÌáÊ¾Í¼±ê
End Type
   
'   dwState   to   NOTIFYICONDATA   structure
Private Const NIS_HIDDEN = &H1           '   Òþ²ØÍ¼±ê
Private Const NIS_SHAREDICON = &H2           '   ¹²ÏíÍ¼±ê
   
'   dwInfoFlags   to   NOTIFIICONDATA   structure
Private Const NIIF_NONE = &H0           '   ÎÞÍ¼±ê
Private Const NIIF_INFO = &H1           '   "ÏûÏ¢"Í¼±ê
Private Const NIIF_WARNING = &H2           '   "¾¯¸æ"Í¼±ê
Private Const NIIF_ERROR = &H3           '   "´íÎó"Í¼±ê
...
Dim PE As PROCESSENTRY32 '´´½¨½ø³Ì¿ìÕÕ
SnapShot = CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS, 0) 'Èç¹û¶ÓÁÐ²»Îª¿ÕÔòËÑË÷
If SnapShot <> -1 Then 'ÉèÖÃ½ø³Ì½á¹¹³¤¶È
PE.dwSize = Len(PE) '»ñÈ¡Ê×¸ö½ø³Ì
NextProcess = Process32First(SnapShot, PE)
Do While NextProcess '¿É¶Ô½ø³ÌÐò×öÏàÓ¦´¦Àí
'»ñÈ¡ÏÂÒ»¸ö
NextProcess = Process32Next(SnapShot, PE)
Loop 'ÊÍ·Å½ø³Ì¾ä±ú CloseHandle (SnapShot)
End If
End Sub
